                         ...
                         //to print out generated 2d arrays due to added vectors
                          ...
                          if (*max_element(doz, doz + 405)>0)
                                ...
                                for (int i=0; i<array.size(); i++)
                                        {
                                        doz[i]=15*doz[i]/ (*max_element(doz, doz + 405));
...
{	
        ...
        auto c15 = new TCanvas("c15","c15",600, 400);
        TMultiGraph  *mg  = new TMultiGraph();

        CSVobject. readCSV("ParticleFieldBF_HSG_LEMIII_LQparameters_LEMIII.csv","dose_BF.csv");        
        TGraph *BF  = new TGraph(405,CSVobject. X_axis,CSVobject.SF);
        BF->SetTitle("BF");
        BF->SetMarkerStyle(29);
        BF->SetMarkerColor(3);

       CSVobject. readCSV("ParticleFieldBG_HSG_LEMIII_LQparameters_LEMIII.csv","dose_BG.csv");
        TGraph *BG  = new TGraph(405,CSVobject. X_axis,CSVobject.SF);
        BG->SetTitle("BG");
        BG->SetMarkerStyle(39);
        BG->SetMarkerColor(2);

        CSVobject. readCSV("ParticleFieldQF_HSG_LEMIII_LQparameters_LEMIII.csv","dose_QF.csv");  
        TGraph *QF  = new TGraph(405,CSVobject. X_axis,CSVobject.SF);
        QF->SetTitle("QF");
        QF->SetMarkerStyle(20);
        QF->SetMarkerColor(7);

        CSVobject. readCSV("ParticleFieldQG_HSG_LEMIII_LQparameters_LEMIII.csv","dose_QG.csv"); 
        TGraph *QG  = new TGraph(405,CSVobject. X_axis,CSVobject.SF);
        QG->SetTitle("QG");
        QG->SetMarkerStyle(49);
        QG->SetMarkerColor(4);

        CSVobject. readCSV("ParticleFieldEM_HSG_LEMIII_LQparameters_LEMIII.csv","dose_EM.csv");  
        ...
        mg->SetTitle("Survival curve in water for different nucleus-nucleus collision models---LEMIII");
