...
{ 
 ...
 for (int i=0;i<5;i++)
    {    
        ...
        g[i]=(TProfile*)f[i]->Get("p" + particleType [6]);
        ...
        g[i]->GetYaxis()->SetTitle("Mean kinietic Energy(MeV/u)");
        ...
        for (int k =1; k<no_charges ; k++) //information of 9 charges to be read to 10 files
                {
                        ...
                        g[i] =(TProfile*)f[i]->Get("p" + particleType [k]);//get profiles
                                                                               //for each file
                                                                               //with name k
                        //TH1F*h =(TH1F*)f->Get(particleType [1]);//get histogramms
                        g[i]->GetXaxis()->SetRangeUser(0, 405);                                       //for each file
                                                                //with name k
                        g[i]->Draw("same PLC PMC HIST");
                        
                        legend[i]->AddEntry("p" + particleType [k],particleType [k]);
  ...
  legend[3]->SetHeader("carbon and mixed field--Legend");
  legend[3]->Draw();
